...
function CheckpointParticipants() {
  const [data, setData] = useState([]);
  const [openCheckpoints, setOpenCheckpoints] = useState({});
  const socketRef = useRef(null);
  ...
  const exportToExcel = (checkpoint) => {
    const rows = checkpoint.visits.map((v) => ({
      Name: v.participant?.name || "",
      Branch: v.participant?.branch || "-",
      Year: v.participant?.year || "-",
      Status: v.lastStatus || "-",
      "Last Scan": v.lastScanTime
        ? new Date(v.lastScanTime).toLocaleString()
        : "-",
    }));
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, checkpoint.name);
    ...
    saveAs(blob, `${checkpoint.name}_participants.xlsx`);
  };
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="bg-blue-500 hover:bg-blue-600 text-white text-sm px-4 py-2 rounded-md shadow"
                >
                  Export
                </button>
                <span
                  className={`transform transition-transform duration-200 ${
                    openCheckpoints[cp.id] ? "rotate-90" : ""
                  } text-gray-500`}
                >
                  ▶
                </span>
